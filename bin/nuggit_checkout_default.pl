...
submodule_foreach(\&checkout_default_branch_in_submodule);

# checkout default branch at root
checkout_default_branch();



# check all submodules to see if the branch exists
sub checkout_default_branch_in_submodule
{
  my $cwd = getcwd();
  my $tmp;
  my $default_branch;
  ...
  $default_branch = $1;  
#  print $tmp;
  print "current dir: $cwd, $current_dir\n";
